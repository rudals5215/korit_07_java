...
/*
    ...
    특징 :
        ...
        2. return 값이 없다 -> 즉, call1() / call2() 유형으로 생각 할 수 있습니다.
        3. 객체 초기화 : 생성자는 객체의 필드 '초기화'를 담당한다.

    생성자의 정의 방식
        1. 기본 생성자 : 매개변수가 없는 생성자
            (사실 클래스를 만들 때 default로 생성되는 형태로 정의하지않아도 될 때가 있음)
            그래서 ClassA, Car에서 기본 생성자를 정의하지 않고
            ClassMainm CarMain에서 객체를 생성해봤었습니다, -> 복습 필요
...
public class Constructor {
    ...
    Constructor(String title){
        System.out.println("RequireArgsConstructor(String 매개변수를 필수로 요구하는 생성자)");
        this.name = title;
    }
