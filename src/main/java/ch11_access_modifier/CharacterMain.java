...
/*
    ...
    attack() -> 공격 시 콘솔 창에
        name이 power로 공격 !
        name이(가) 경험치 amount을(를) 얻었습니다. -> 라고 두 줄 출력되게 작성할 것, call1()유형
    heal() -> 체력을 10 회복하고, 현재 체력 출력
        체력이 10 회복되었습니다,. 현재체력ㄱ : health(숫자로 출력돼야 함)
...
class Character{
    ...
    // 생성자 (캐릭터 생성 시에 필수적으로 요구되는 애들만 매개변수로 받은 매개변수 생성자)
    ...
    public void attack(){
        ...
        // 공격하고 나서 경험치를 얻는다. -> 그런데 외부에서 공격 이외의 방식으로 경험치를
        // 강제적으로 획득하는 버그는 막는다. 그렇다면 위의 가이드에서 작성했던 gainExp()는 외부에서 실행하면 안됨.
        this.gainExp(30);
    }
...
public class CharacterMain {
    public static void main(String[] args) {
        ...
        // 객체의 필드에 값 대입
//        warrior.name = "전사" ;   //field가 public이기 때문에 타 클래스가
//        warrior.setName("전사");      // 이상의 직접 대입 방식과의 코드 작성 차이점애 대해서 확인하기
//        warrior.setHealth(100);
//        warrior.setPower(200);
//        warrior.setSkill("세로베기");
//        warrior.setExp(30);
        System.out.println(warrior.getHealth());    // 생성자 내부에 정의했기 때문에
...
/*
    이상의 코드에서 여러분들이 중점적으로 학슴해야할 점은
    접근 지정자의 설정 수준에 따라서 setter / getter가 필요한 사례가 있고 아닌 사례가 있다는 점 입니다.
    예를 들어 게임을 하거나 SNS를 한다고 쳤을 때 닉네임의 경우는 누구나 볼 수 있죠.
    그렇다면 닉네임 필드는 public으로 설정되어 있을겁니다.
    그렇기 때문에 getter를 통한 닉네임의 호출은 필요가 없어서 이상의 코드에서 getName()메서드는 쓸모가 없을겁니다.

    우리는 어떤 클래스/ 필드 / 메서드를 어떤 접근지정자를 설정해야 할지도 고민해야한다
