...
//        System.out.println(i);          // 결과값 : 1
        ...
        // 현재 상황에서 배열 내부의 값을 확인 할 수가 없습니다. 주소지만 출력이 됩니다.
        // 배열 내부의 각 숫자들은 뭐라고 한다? -> 원소
        // 현재 배열 내부에는 다수의 데이터가 있기 떄문에 콘솔창에서 전체 확인이 불가능합니다.
        // 즉 element들을 확인하기 위해서는 배열 내부로 직접 탐색하는 과정이 필요합니다.
        // 이때 필요한 개념이 index(주소지라고 하기도 합니다) 개념입니다.
        ...
        // 어, index만 빼고 나머지가 반복되네? 반복문 써봐야 겠네? 하면 개발자 지망생
        // 반복문을 활용하여 arr01의 내부 element들을 콘솔에 출력하시오.
        for (int num = 0;num < 5 ; num++){      // 5가 하드코딩 되어있음. -> literal을 그대로 썼다
            System.out.print(arr01[num]+" ");       // num이 어디에 적용되어야 하는지 감안할 필요가 있다.
        }
        System.out.println();
        for(int num = 0; num < arr01.length ; num++){    // arr01.length 형태로 배열 내부의 데이터 개수가 바뀌더라도 문제 없음
            System.out.println(arr01[num] + " / ");
        }

        "안녕하세요".length();

        // 근데 우리가 필드를 참조할 때 객체명.필드 형태로 참조했었습니다.
        // bank1.accountHolder와 같은 방식으로 참조가 가능했습니다.
        ...
        // 이상의 모든 것을 종합했을 때.
        // 배열 내부의 방의 개수 / 주소지의 한계값 -1을 우리가 할상 알고 있는 것은 아니기 때문에
        // element 값을 출력하기 위한 반복문의 한계값을 설정할 때
        // 배열명.length를 쓰는것을 생활화하자.

        // arr01 내부에 있는 1,2,3,4,5는 굳이 따지자면 arr01[0],arr01[1],...,arr01[4]라는 변수명을 지니고 있다고 볼 수 있다.

        // 변수의 개념을 우리는 다시 생각해야 합니다.
        arr01[0] = 10;      // 그럼 얘는 재대입이 겠네요.
        System.out.println(arr01[0]);       // 결과값 : 10

        // 그럼 우리는 배열에 값을 대입할 수가 있습니다.
        // arr02는 현재 10개짜리 방을 지닌 배열에 해당하고, 내부에는 아무런 값이없습니다.
        for (int i = 0; i < arr02.length; i++){
            System.out.println(arr02[i]);
        }

        // arr02에 21,22,23,24,25,...,30을 대입하고
        // 21 / 22 / 23 / 24 / ... / 30이라고 출력될 수 있도록 코드를 작성하시오.
//        for (int i = 0; i < arr02.length; i++){
//            for(int j = 20; j < arr02.length + 20; arr02[i] = ++j){
...
//        }
        ...
        // 그래서 다른 자료형의 배열이라더라도 기본적으로 elemennt를 추출하는 절차는 동일합니다.
