...
/*
    배열의 출력
    엄밀히 말하자면 저희는 배열을 출력한ㄷ게아니라 여태까지 배열 내부의 element들을 출력했습니다.
    System.out.println(arr01); 과 같은 출력문을 작성했을 때
    주소값만 나온다는 사실을 확인할 수 있었습니다.

    근데 element의 값을 조정하거나, 더하거나와 같은 추가적인 가공을 하기 위해서 for 반복문을
    사용해왔는데, 귿이 변형이 없고 배열 자체를 확인하고 싶을 때마다
    main에 반복문 작성하든지 method를 정의하든지 하는 일은 번거로은 일이 됩니다.

    Arrays 츨래스를 사용하고, '정적' 메서드인 .toString(배열명)을 사용하면
    배열 전체를 출력할 수 있습니다.

    여기서 우리는 클래스명.메서드명()이라는 점에서 주목할 필요가 있습니다.
    scanner.nextLint()과 다릅니다.
...
public class Array12 {
    public static void main(String[] args) {
        ...
        // int[]numbers 배열에 1,2,3,4,5,... 10RKwl eodlqgo entpdy.
        ...
        /*
            ...
            이상의 과정이 overloading이 이루어 졌다는 점을 파악할 수 있으면 현재 수준으로는
            만족스럽겠습니다.
            그래고 두번째 argument로 Comparator.reverseOrder();가 사용되었는데.
            Comparator.reverseOrder()의 결과값 / return값이 sort()메서드의
            두번째 argument로 사용되었다는 점에서 역시 함수형 프로그래밍의 일종이라고
            볼 수 있겠네요
            근데 Comparator.reverseOrder()메서드를 쓰려면
            기본 자료형(원시 자료형 / primitive type) int[]배열을 사용할 수 없어서
            Integer[] 배열을 사용하였는데, 현재는
            char -> String 처럼 int -> Integer 라고만 생각해두시면 되겠습니다.
            추후 수업 예정
         */
    }
}
