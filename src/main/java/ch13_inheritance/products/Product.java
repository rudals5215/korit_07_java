...
/*
    ...
    지금 작성하는 부분의 경우 github에 완전판이 올라가기 때문에 왜 오류ㅜㅠ가 발생하는지
    어떻게 검증하는지 어려움을 겪을 수있습니다.

    부모 - 자식의 상속관계가 성립되어있을 때,
...
public class Product extends Item{
    ...
    public void showInfo(){
        System.out.println("제품명 : " + this.getName());          // 부모 거라 getter로 조회
        System.out.println("카테고리 : " + this.getCategory());
        System.out.println("가격 : " + price);                    // 자식 거라 field로 조회
        System.out.println("재고 : " + stock);                    // 두줄씩 코드라인의 차이가 생기는 이유애 대해 명확히 아시길 추천
    }
}
