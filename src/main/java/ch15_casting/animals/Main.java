...
public class Main {
    public static void main(String[] args) {
        ...
        dog1.makeSound();
        dog1.fetch();
        animal1.makeSound();        // 결과값 : 개가 짖습니다. 즉 재정의된 method가 호출 됨
...
//        animal2.fetch();        // 불가능
        ...
        /*
            클래스A 객체명 : new 클래스B();를 통해서 객체를 생성했을 경우
            1. Override가 적용 된 메서드, 즉 자식 클래스의 메서드가 호출 됨.
            2. 클래스B(자식 클래스)의 고유 메서드는 호출 불가능.

            그럼 어차피 재정의된 method버전으로 호출이 될거라면 애초에
            클래스B 객체명 = new 클래스B();  로 생성해버리면 method도 호출되고 고유 method도
            호출할 수 있으니까 골치안아픈거 아니냐
         */
        ...
        /*
            ...
            방법
            (클래스명) 객체명; -> (int)2.3; 과 같은 방식이죠. conversion의 객체 버전이라고 생각하시면 됩니다ㅓ.
            그래서 기본 자료형의 형 변환에서 벗어나서 참조 자료형까지 고려 하ㅓ셔야 합니다.
         */

        Dog dog2 = (Dog) animal2;    // 다운캐스팅 방법 -> 위에 보시면 Animal animal2 = new Dog();으로 생성해서  dog2 객체명은 없습니다.
        dog2.makeSound();
        dog2.fetch();
        // dog2로 새로은 객체명을 선언하고 거기에 명시적 참조 자료형 캐스팅을 적용하여 Animal 클래스의
        // 인스턴스인 animal2를 downcasting 했습니다.
        // 그 결과 원래도 되던 재정의 버전의 makeSound(); 호출른 변함이 없고ㅓ.
...
//        dog3.makeSound();         // 그러니까 얘는 지금 호출도 못해보는 상황입니다.
        /*
            ...
            (A a = new A();이기 때문에)
            하위 클래스의 인스턴스로 다운캐스팅 하는 것이 불가능 합니다.

            이상을 해결하기 위해서는
         */
        Animal animal4 = new Dog();
        // 형태로 선언된 클래스와 호출되는 생성자가 서로 달라야 합니다.(그리고 둘은 상속관계여ㅑ야합니다.)
        ...
        if (animal4 instanceof Dog) {       // 이 조건식이 true라면 animal4의 생성 시 호출된 생성자는 Dog();
            Dog dog4 = (Dog) animal4;       // 그러므로 91번 라인의 실행문을 통해 다운캐스팅을 하더라도 오류발생 x

            dog4.makeSound();
            dog4.fetch();
        }
        System.out.println("animal3는 다운캐스팅 안되는거 알았으니까 검증 후에 다운캐스팅 시도 해야겠네요");
