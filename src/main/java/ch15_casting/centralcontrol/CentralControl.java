...
public class CentralControl {
    ...
    public void addDevice(Power device){
        /*
             지금 우리는 Power의 자식 클래스에 해당하는 인스턴스들을 deviceArray라는 필드에 추가하고 싶습니다.
             그러면 CentralControlMain에서 객체를 생성을 할 때 deviceArray의 최종 indexNumber는 고정되게 되는데,
    ...
    public void powerOn(){
        /*
            해당 클래스읜 Power[] 배열 내에 있는 객체들은 기본적으로
        ...
        for (int i = 0 ; i < deviceArray.length ; i++){
            if (deviceArray[i] == null){
                ...
                continue;
                /*
                     break; 는 반복문을 즉시 종료하고, return;은 method를 즉시 종료하는데,
                     continue;의 경우에는 현재 반복문만 종료하고 그 다음 반복문으로 넘어갑니다.
                     즉 deviceArray[3] 주소지에 객체가 없어 null이 있다면 67번 라인의 조건문이 실행될텐데
                     "장치가 없어 전원을 켜지 않았습니다."가 출력되고 곧장 deviceArray[4]를 확인하게 됩니다.
                 */
    ...
    /*
        ...
        deviceArray를 반복 돌려서 몇 번 인덱스에 어떤 클래스의 객체가 있는지 표시하는 showInfo 메서드를 정의하시오.
        실행 예
        슬롯 [ 1 ] 번 : Computer
        슬롯 [ 2 ] 번 : LED
        ...
    ...
    /*
        이제 배열 내부를 돌면서 각 element들의 고유 메서드들을 실행시켜볼 겁니다.
        논리적으로는 말이 안됩니다...
     */
    ...
    // Main 에서 printer1을 centralControl에 추가, on() / off() 를 실행한 후에
