...
public class Samsung {
    ...
    // 일반 필드 (멤버 변수라는 표헌을 쓰기도 함)
    @Getter             // getCompany()만 만들겠다는 의미
    private String company;     // = getClass().getSimpleName(); 도 가능하다
    private int serialNumber;

    // private으로 생성자를 선언
    private Samsung(){
        company = getClass().getSimpleName();       // 어차피 이거 결과값 Samsung입니다.
        serialNumber = 20250000;                    // 즉 객체 생성시에 필드값을 초기화 시켰습니다.
    }
    ...
    public String createSerialNumber(String model){
        ...
        /*
             싱글톤으로 작성했으니까 어차피 객체 하나만 생성될거고
             Main 단계에서 다수의 객체에서 createSerialNumber()를 호출하더라도
             전혀 serialNumber가 중복되지 않을거라고 볼 수있습니다.
             그러면 정적변수로 선언하고 다수의 삼성 객체를 만들어도 serialNumber가 안겹치지 않을까요?
             라고 질문한다면 맞는 말입니다.
