...
public class StrMap {
    public static void main(String[] args) {
        ...
        /*
            ...
            하지만 Map의 경우 아까 출력 결과에서 본 것처럼 순서를 보장하지 않기 때문에
...
//        koreanscores1.put("김일",new Double[2]);
        ...
        // 특정 key의 value 수정
        // # 1. 아까 말한것 처럼 key에는 value가 하나밖에 없기 때문에 최근걸로 덮어쓰기가 됩니다
        System.out.println(koreanscores2);
        koreanscores2.put("김오",4.7);
        System.out.println(koreanscores2);

        // # 2. .replace();
        koreanscores2.replace("김칠",78.2);
        System.out.println(koreanscores2);
        /*
            두 가지 방법이 있는 이유는 .put()의 경우에는
            없는 key 라면 key를 새로 생성해서 value를 초기화 하겠지만
            있는 key 라면 value에 재 대입을 하게 됩니다.
        ...
        System.out.println("김육 학생의 존재 여부 : "+ searchKeyFlag1);

        // 특정 value의 존재 여부 -> containsValue(); -> boolean
        boolean searchValueFlag1 = koreanscores2.containsValue("김삼");
        System.out.println("김삼 존재 여부 : " + searchValueFlag1);

        // Map의 엔트리로부터 Set을 생성
        Set<Map.Entry<String,String>> entrySet1 = new HashSet<>();
        // 이상의 코드는 Set를 하나 만드는데, 내부에 key가 String, value가 String인 Map이 통째로 들어간다는 것을 의미합니다.
...
//        List<Map.Entry<String,String>> entryList2 = strMap1.entrySet();
        // Map -> List 로의 직접 변환이 불가능하다는 걸 보여주는 예시

        entryList1.addAll(entrySet2);
        System.out.println(entryList1);     // List로 바뀌었으니까 element 추출이 가능.
        // 이상까지의 과정에서 Map -> Set -> List로 바꾸는 과정을 코드고 작성했는데
        // Map -> List로 바로 가는게 가능한지 여부를 따질 필요가 있습니다.
        ...
        // value Set 출력
//        Set<String> valueSet = strMap1.values();  이게 불가능한 이유 = 중복이 가능하기 때문에 중복제거를 하여 element가 줄어들게 됨
        /*
             이상에서 강제 형변환을 하지 않은 상태에서 오류가 발생하는 이유는 key는 어차피 중복을 허용하지 않기 때문에
             Set으로 바꾸는게 가능한 반면, 복수의 key에서 동일한 value가 존재할 수 있기 때문에 직접적으로 Set으로 바꾸지 않습니다.
         */
        Collection<String> values = strMap1.values();
        System.out.println(values);
        // 저희는 Collection 자료형은 또 처음 써봤습니다.
