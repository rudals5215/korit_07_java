...
public class JSON2 {
    public static void main(String[] args) {
        ...
        /*
            ...
            그리고 JSON1.java 파일을 참조해서 json 데이터로 바꿔 보시오.
         */
        ...
        String jsonProductMap1 = gson.toJson(productMap1);
        ...
        Map<String , String> productMap2 = gson.fromJson(jsonProductMap2, Map.class);
        ...
        System.out.println(userMap2);           // Map 클래스로 출력
        /*
            ...
            JsonData(즉 String Data)를 Map 으로 바꾸는 겁니다.
            String의 경우에는 개행을 통해서 prettyPrinting이 적용될 수 있지만,
            Map은 toString() 메서드를 재정의 하지 않으면 default값 형태로 콘솔에 출력이 되겠죠.
            그래서 gson.fromJson()이든 gsonBuilder.fromJson()이건 콘솔에 찍히는 결과값은 동일하다.
         */

        /*
            그렇다면 이상의 코드처럼 Java 객체로 만든 것을 -> Json 데이터로 바꾼 다음에 -> 그걸 Map 형태로도 할 수 있겠네요.
            하지만 Map의 경우 Json 으로 바꾸는 것까지는 자유인데 걔를 Java 객체로 만들고자 한다면 클래스가 정의되어야 합니다.
            객체는 클래스라는 설계도를 기준으로 만들어진 거니까요.

            json -> jsonObject로 바꿀겁니다.
            그 말은 json =/= jsonObject 라는 뜻이 되겠네요.

            잘 생각해보면 jsonUser1을 선언할 때 자료형을 보면 String 으로 잡았습니다.
