...
/*
    ...
    i < strList.size()를 쓰지 않고 i < 3001과 같은 방식으로 하드코딩을 했을 때에만 예외가 발생하겠죠.
...
//        person1.school = "코리아아이티";        // IDE 상에서 예외 처리가 아니라 빨갛게 나오는 예시 # 1
        person1.showInfo();
//        person1.displayInfo();                // IDE 상에서 예외 처리가 아니라 빨갛게 나오는 예시 # 2
...
//        int b = 0;                  // 나누는 수
//        int result = a / b ;
//        System.out.println(result);     // 0으로 나눌 수 없다는 오류 발생
...
//        int b = 0;
//        try{
//            int result = a / b;     // ArithmeticException 예외 발생 지점 -> 37번 라인이 아니라 catch 문으로 넘어갑니다.
...
//        }
        /*
            ...
            일단 실행을 시켜서 오루가 나는 것을 확인해야 하는가?
         */

        // 배열 인덱스 예외
        int [] numbers = {1,2,3};
//        System.out.println(numbers[5]);     //ArrayIndexOutOfBoundsException: Index 5 out of bounds for length 3
        // 배열 인덱스 예외 -> 복수 catch 문 사용
        try {
            System.out.println(numbers[5]);
        } catch (ArrayIndexOutOfBoundsException e){
            System.out.println(e.getMessage());
        }catch (Exception e) {                      // 예외 중에 가장 상위 예외 클래스에 해당함.
            throw new RuntimeException(e);          // 사실 저희가 보던 시뻘건게 이 오류 메세지 였습니다.
        }
//        catch (ArrayIndexOutOfBoundsException e){ // 이렇게 사용은 불가능
//            System.out.println(e.getMessage());
//        }
        finally {
            System.out.println("프로그램 정상 종료");
        ...
        /*
            이상의 코드 라인에서 중요한 점은 catch 문이 복수로 나올 수 있다는 점입니다.
