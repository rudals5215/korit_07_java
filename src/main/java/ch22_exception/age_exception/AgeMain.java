...
public class AgeMain {
    // 예외 클래스는 별개로 정의했지만 메서드를 호출했을 때 예외를 처리해야 하기 때문에 여기에 추가적으로 메서드 정의(아까 login()처럼)
    public static void checkAge(int age) throws InvalidAgeException{
        if (age < 0 || age > 200){
            // 조건에 맞지 않는 경우 사용자 정의 예외 발생시킬 것
            // 예외를 발생시킨다는 의미 -> 예외 객체를 생성하는다느 의미 입니다.
...
//        int age = scanner.nextInt();    // 전에는 여기에 바로 작성해서 넣었는데,
        // 생각해보면 여기 부분에 음수값이나 300 초과 정수를 집어넣을 수 있습니다.
        // 혹은 double 자료형을 넣을 수도 있겠네요ㅕ.
        // 예외가 발생할 가능성이 있는 부분을 try문 안에 넣어주는 것이 중요합니다.
        try{
            int age = scanner.nextInt();
            checkAge(age);  // 예외를 던질(throws) 가능성이 있는 메서드 호출
        ...
        /*
            ...
            예외가 발생하지 않는다면 예외 관련 객체가 생성되지 않고, catch{} 문은 실행되지 않습니다.
            예외가 발생한다면
            예외 발생 시점 이후의 try{} 문은 실행되지 않고 곧장 catch{}문이 실행 됩니다.
